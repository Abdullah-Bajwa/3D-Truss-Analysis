...
function [obj] = truss3D(inputfile)
% Purpose:
...
u  =zeros(sDof,1);
F = zeros(sDof,1);      % global displacement and load vectors
axialforce =zeros(nElems,1);   % result vectors
stress = zeros(nElems,1);   % result vectors
strain = zeros(nElems,1);   % result vectors
